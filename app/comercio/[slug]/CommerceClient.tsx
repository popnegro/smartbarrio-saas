...
export function CommerceClient({commerce}:{commerce:Commerce}) {
  ...
  return <div className="site-wrap">
    ...
    <main className="public-main">
      ...
      <div className="content-grid"><div className="catalog-grid">{commerce.products.map(p=><button className="catalog-card" key={p.slug} onClick={()=>setSelected(p)}><div className={'catalog-img '+commerce.accent}>{productImage(p)?<img src={productImage(p)} alt="" aria-hidden="true"/>:<span>{p.imageLabel}</span>}</div><div><h3>{p.name}</h3><p>{p.price||'Consultar'} · {p.description}</p></div><ArrowUpRight size={16}/></button>)}</div><div className="service-list"><div className="service-head">Servicios</div>{commerce.services.map(s=><div className="service-row" key={s}><span>{s}</span><ChevronRight size={16}/></div>)}</div></div>
